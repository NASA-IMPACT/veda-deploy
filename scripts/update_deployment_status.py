...
COMPONENTS = [
    (
        ...
        "BACKEND",
        "https://github.com/NASA-IMPACT/veda-backend",
    ),
    (
        "features_api",
        "DEPLOY_FEATURES_API",
        ...
        "FEATURES_API",
        "https://github.com/NASA-IMPACT/veda-features-api-cdk",
    ),
    (
        "routes",
        "DEPLOY_ROUTES",
        "VEDA_ROUTES_GIT_REF",
        "ROUTES",
        "https://github.com/NASA-IMPACT/veda-routes",
    ),
    (
        "sm2a",
        "DEPLOY_SM2A",
        ...
        "SM2A",
        "https://github.com/NASA-IMPACT/veda-sm2a",
    ),
    (
        "monitoring",
        "DEPLOY_MONITORING",
        "VEDA_MONITORING_GIT_REF",
        "MONITORING",
        "https://github.com/NASA-IMPACT/veda-monitoring",
    ),
    (
        "s3_dr",
        "DEPLOY_S3_DISASTER_RECOVERY",
        "VEDA_S3_DISASTER_RECOVERY_GIT_REF",
        "S3_DR",
        "https://github.com/NASA-IMPACT/s3-disaster-recovery",
    ),
    (
        "titiler_multidim",
    ...
    (
        ...
        "TITILER_CMR_GIT_REF",
        "TITILER_CMR",
        "https://github.com/developmentseed/titiler-cmr",
    ),
]
...
def make_ref_link(repo_url: str, ref: str) -> str:
    return f"[{ref}]({repo_url}/tree/{ref})" if ref else ""
...
def write_state(path: str, state: dict):
    ...
    with Path(path).open("w", encoding="utf-8") as f:
        f.write(
            "# Deployment Status\n\nIf a component or environment is not listed, "
            "it has not yet been deployed through veda-deploy.\n\n"
        )
...
def collect_updates_from_env() -> dict:
    ...
    for _, deploy_env, ref_env, display, _ in COMPONENTS:
        ...
        if (
            deploy_flag == "true" and ref_val.strip()
        ):  # this is weird but GHA won't let the flag be cast as a boolean
...
def main():
    ...
    ap = argparse.ArgumentParser(
        description="Update deployment-status.md with current refs."
    )
    ap.add_argument(
        "--env",
        required=True,
        help="Target environment name (must match your GH environment)",
    )
    ap.add_argument(
        "--file", default=DEPLOYMENT_FILE, help="Path to deployment status markdown"
    )
    args = ap.parse_args()
