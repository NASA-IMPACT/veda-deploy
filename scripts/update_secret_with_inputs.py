...
def update_secret_with_inputs():
    parser = argparse.ArgumentParser(
        description="""
        **WARNING** This is destructive, if an input is provided that's name
        already exists in the AWS Secret, it will be overridden.

        ---

        Takes in N inputs in the form --input-name=value and inserts them
        into the provided AWS SecretsManager secret. If a prefix is provided,
        it will be appended to the input name

        This assumes that the SecretString value is a stringified JSON object

        With no prefix, an input of --my-secret-item=hello will be inserted as:

        MY_SECRET_ITEM=hello

        With a prefix of MY_PREFIX, input --my-secret-item=hello will be inserted as:
    ...
    parser.add_argument(
        ...
        default="",
        required=False,
        help="Optional prefix to append to inputs names",
    )
    parser.add_argument(
        "--secret-id",
        required=True,
        help="The ARN or Name of the AWS SecretsManager secret to update",
    )
    ...
    for arg in unknown_args:
        if match := re.match(_ARG_REGEX, arg):
            secret_entry_name = known_args.prefix.upper() + match.group(
                1
            ).upper().replace("-", "_")
