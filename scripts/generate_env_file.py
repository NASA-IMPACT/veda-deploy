...
def get_cf_outs_as_env(stack_names, out_file):
    ...
    for stack_name in stack_names.split(","):
        ...
        with Path(out_file).open("a") as _env:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="stack_names",
        help=(
            "Cloudformation Stack names (comma separated). "
            "If the flag is used without a value, or if the flag is omitted "
            "entirely, it defaults to None."
        ),
        required=False,
        default=None,
        nargs="?",
    ...
    secret_id, stack_names, env_file = (args.secret_id, args.stack_names, args.env_file)

    generate_env_file(stack_names=stack_names, secret_id=secret_id, out_file=env_file)
