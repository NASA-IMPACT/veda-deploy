...
def _get_link(obj: dict, rel: str) -> str:
    """
    Helper function to find a specific link
    using its 'rel' type in a STAC object's "links" array.
    """
...
def test_collection_pagination_next_link_is_valid():
    ...
    the 'next' link provided in the item list is a valid URL that returns a 200 status
    and does not contain a CloudFront cache error.

    This test fetches all collections and then individually checks their item lists
    for pagination links. It focuses on the custom host configuration as it's the
    primary way
    ...
    if not custom_host:
        pytest.skip(
            "VEDA_CUSTOM_HOST environment variable not set. Skipping pagination test."
        )
    ...
    try:
        ...
        # Fail fast if collections endpoint is down
    ...
    for collection_summary in collections:
        ...
        # Get the link to the collection's items.
        # The 'items' link is within the collection's own links.
        ...
        try:
            ...
            if items_response.status_code != 200:
                print(
                    f"Warning: Could not fetch items for '{collection_id}'. "
            ...
            print(
                f"Warning: Could not fetch or parse items for "
                f"'{collection_id}'. Error: {e}"
            )
            continue
        ...
        if next_link and next_link.get("href"):
            ...
            try:
                ...
                assert next_page_response.status_code == 200, (
                    f"'next' link for {collection_id} failed with status "
                    f"{next_page_response.status_code}"
                )
                print(
                    f"  - Success! 'next' link returned status "
                    f"{next_page_response.status_code}."
                )

                x_cache_header = next_page_response.headers.get("x-cache", "").lower()
                assert "error from cloudfront" not in x_cache_header, (
                    "CloudFront error detected in x-cache header for "
                    f"{collection_id}: {x_cache_header}"
                )
                print("  - Success! No CloudFront error found in x-cache header.")

                # Once we've successfully tested one 'next' link,
                # we can exit to keep the test fast.
                break
            except requests.exceptions.RequestException as e:
                pytest.fail(
                    f"Request for 'next' link URL {next_url} failed. Error: {e}"
                )
        else:
            print(
                f"  - No 'next' link found for '{collection_id}' "
                "(collection may have less than one page of items)."
            )
